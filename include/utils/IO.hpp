...
namespace io {
    ...
    inline void saveReservationTabular(const std::string &filename, const ReservationTabular &reservationTabular) {
        saveToFile(filename, reservationTabular.to_json());
    }

    inline ReservationTabular loadReservationTabular(const std::string &filename) {
        nlohmann::json jsonData = loadFromFile(filename);
        ReservationTabular reservationTabular;
        reservationTabular.from_json(jsonData);
        return reservationTabular;
    }
}